<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-light text-stone-800">Guisados</h1>
      <button @click="abrirNuevo" class="inline-block bg-stone-800 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-stone-700 transition-colors">+ Nuevo guisado</button>
    </div>
    ...
    <div v-else class="border border-stone-200 rounded-lg overflow-hidden bg-white">
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-stone-100">
          <tr v-for="g in guisados" :key="g.id" class="hover:bg-stone-50 transition-colors">
            <td class="px-4 py-3 text-stone-800">{{ g.nombre }}</td>
            <td class="px-4 py-3">
              <span class="text-xs bg-stone-100 text-stone-600 px-2 py-0.5 rounded capitalize">{{ g.categoria.toLowerCase() }}</span>
            </td>
            <td class="px-4 py-3 text-right">
              <button @click="editar(g)" class="text-stone-500 hover:text-stone-700 text-xs font-medium mr-3 transition-colors">Editar</button>
              <button @click="eliminar(g.id)" class="text-red-500 hover:text-red-700 text-xs font-medium transition-colors">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50" @click.self="cerrarModal">
      <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4 border border-stone-200">
        <h2 class="text-lg font-medium text-stone-800 mb-4">{{ editando ? 'Editar guisado' : 'Nuevo guisado' }}</h2>
...
const form = ref({ nombre: '', categoria: 'POLLO' })

function abrirNuevo() {
  editando.value = null
  form.value = { nombre: '', categoria: 'POLLO' }
  showModal.value = true
}

function editar(g) {
  editando.value = g.id
  form.value = { nombre: g.nombre, categoria: g.categoria }
  showModal.value = true
}
...
async function guardar() {
  try {
    if (editando.value) {
      await api.put(`/guisados/${editando.value}`, form.value)
    } else {
      await api.post('/guisados', form.value)
    }
    await cargar()
    cerrarModal()
  } catch (e) {
...
async function eliminar(id) {
  if (!confirm('¿Eliminar este guisado?')) return
  try {
    await api.delete(`/guisados/${id}`)
    await cargar()
  } catch (e) {
...
async function cargar() {
  loading.value = true
  try {
    const { data } = await api.get('/guisados')
    guisados.value = data
    errorMsg.value = ''
  } catch (e) {
