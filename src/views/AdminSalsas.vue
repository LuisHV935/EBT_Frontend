<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-light text-stone-800">Salsas</h1>
      <button @click="abrirNuevo" class="inline-block bg-stone-800 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-stone-700 transition-colors">+ Nueva salsa</button>
    </div>
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="bg-stone-50 border-b border-stone-200">
            ...
            <th class="text-left px-4 py-3 font-medium text-stone-600">Tipo</th>
        ...
        <tbody class="divide-y divide-stone-100">
          <tr v-for="s in salsas" :key="s.id" class="hover:bg-stone-50 transition-colors">
            <td class="px-4 py-3 text-stone-800">{{ s.nombre }}</td>
            <td class="px-4 py-3">
              <span class="text-xs bg-stone-100 text-stone-600 px-2 py-0.5 rounded">{{ s.tipo || '—' }}</span>
            </td>
            <td class="px-4 py-3 text-stone-400 text-xs max-w-xs truncate">{{ s.descripcion || '—' }}</td>
            <td class="px-4 py-3 text-right">
              <button @click="editar(s)" class="text-stone-500 hover:text-stone-700 text-xs font-medium mr-3 transition-colors">Editar</button>
              <button @click="eliminar(s.id)" class="text-red-500 hover:text-red-700 text-xs font-medium transition-colors">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50" @click.self="cerrarModal">
      <div class="bg-white rounded-lg p-6 w-full max-w-md mx-4 border border-stone-200">
        <h2 class="text-lg font-medium text-stone-800 mb-4">{{ editando ? 'Editar salsa' : 'Nueva salsa' }}</h2>
        <form @submit.prevent="guardar" class="space-y-4">
          ...
          <div>
            ...
            <select v-model="form.tipo" class="w-full border border-stone-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-stone-400">
...
const form = ref({ nombre: '', tipo: '', descripcion: '' })

function abrirNuevo() {
  editando.value = null
  form.value = { nombre: '', tipo: '', descripcion: '' }
  showModal.value = true
}

function editar(s) {
  editando.value = s.id
  form.value = { nombre: s.nombre, tipo: s.tipo || '', descripcion: s.descripcion || '' }
  showModal.value = true
}
...
async function guardar() {
  try {
    if (editando.value) {
      await api.put(`/salsas/${editando.value}`, form.value)
    } else {
      await api.post('/salsas', form.value)
    }
    await cargar()
    cerrarModal()
  } catch (e) {
...
async function cargar() {
  loading.value = true
  try {
    const { data } = await api.get('/salsas')
    salsas.value = data
    errorMsg.value = ''
  } catch (e) {
